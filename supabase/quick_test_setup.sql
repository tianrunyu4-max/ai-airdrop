...
BEGIN
  -- 创建 test_admin (创世用户)
  INSERT INTO users (username, password, is_agent, u_balance, points_balance, invite_code, direct_referral_count)
  VALUES ('test_admin', crypt('123456', gen_salt('bf')), true, 100, 0, 'ADMIN001', 0)
  ON CONFLICT (username) DO UPDATE 
  SET u_balance = 100, points_balance = 0
  RETURNING id INTO v_admin_id;

  -- 创建 test_user1 (test_admin的下线)
  INSERT INTO users (username, password, is_agent, u_balance, points_balance, invite_code, parent_id, direct_referral_count)
  VALUES ('test_user1', crypt('123456', gen_salt('bf')), true, 100, 0, 'USER0001', v_admin_id, 0)
  ON CONFLICT (username) DO UPDATE 
  ...
  INSERT INTO users (username, password, is_agent, u_balance, points_balance, invite_code, parent_id, direct_referral_count)
  VALUES ('test_user2', crypt('123456', gen_salt('bf')), true, 100, 0, 'USER0002', v_admin_id, 0)
  ON CONFLICT (username) DO UPDATE 
  SET u_balance = 100, points_balance = 0, parent_id = v_admin_id
  ...
  RAISE NOTICE '   • test_admin (密码: 123456, 余额: 100U)';
  RAISE NOTICE '   • test_user1 (密码: 123456, 余额: 100U)';
  RAISE NOTICE '   • test_user2 (密码: 123456, 余额: 100U)';
END $$;
...
-- 查看所有用户余额
-- SELECT username, u_balance, points_balance, direct_referral_count FROM users ORDER BY created_at DESC;
